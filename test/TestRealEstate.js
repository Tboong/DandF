...
contract('RealEstate', function(accounts){
    ...
    it("가나슈 두번째 계정으로 매물 아이디 0번 매입 후 이벤트 생성 및 매입자 정보와 buyers 배열 테스팅", function(){
        return RealEstate.deployed().then(function(instance){
            RealEstateInstance = instance;
            return RealEstateInstance.buyRealEstate(0, "sh", 13, {from:accounts[1], value: web3.toWei(1.50,"ether")})
            ...
            assert.equal(receipt.logs[0].args._buyer, accounts[1], "매입자가 가나슈 두번쨰 계정이 아닙니다");
            ...
            assert.equal(web3.toAscii(buyerInfo[1]).replace(/\0/g, ''), "sh", "매입자의 이름이 sh가 아닙니다.")
